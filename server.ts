  ...
  try {
    ...
    if (token) {
      ...
      if (response.ok) {
        ...
        console.log(`[GitHub API] Veřejně nalezeno ${jsonFiles.length} JSON souborů v recipes/. Stahuji...`);
...
app.all("/api", async (req, res) => {
  ...
  try {
    ...
    for (const r of recipesList) {
      ...
      const sha = matchingRemote ? matchingRemote.sha : undefined;

      try {
        console.log(`[GitHub API] Zapisuji recept: ${path} (SHA: ${sha || "nový"})`);
        await octokit.repos.createOrUpdateFileContents({
          owner,
          repo,
          path,
          message: `Uložení receptu '${r.title}' [auto-sync]`,
        ...
        console.error(`Nepodařilo se uložit recept ${fileName}:`, err);
      }
    }

    let deletedCount = 0;
    for (const file of remoteFiles) {
      if (file.type === "file" && file.name.endsWith(".json") && !localSlugs.has(file.name)) {
        try {
          console.log(`[GitHub API] Odstraňuji přebytečný recept: ${file.name}`);
          await octokit.repos.deleteFile({
            owner,
            repo,
            path: `recipes/${file.name}`,
            message: `Odstranění přebytečného receptu '${file.name}' [auto-sync]`,
            sha: file.sha,
            branch,
          });
          deletedCount++;
        } catch (err) {
          console.error(`Nepodařilo se smazat přebytečný soubor ${file.name}:`, err);
        }
      }
    }
    ...
    return res.json({
      success: true,
      message: `Synchronizace dokončena. Úspěšně uloženo ${successCount} receptů a odstraněno ${deletedCount} starých souborů ve složce 'recipes/' na GitHubu!`
    });
  } catch (error: any) {
    console.error("Chyba při POST/PUT /api:", error);
    return res.status(500).json({
      error: `Chyba při zápisu receptů do GitHub složky: ${error.message || error}`
    });
  }
});
